...
_CATEGORY_KEYWORDS: dict[str, list[str]] = {
    "Obst": ["apfel", "birne", "banane", "orange", "zitrone", "limette", "erdbeere",
             "himbeere", "blaubeere", "traube", "kirsche", "pfirsich", "mango",
             "ananas", "melone", "kiwi", "pflaume", "weintraube", "mandarine"],
    "Gemüse": ["tomate", "gurke", "paprika", "zwiebel", "knoblauch", "karotte",
               "möhre", "zucchini", "brokkoli", "blumenkohl", "spinat", "salat",
               "kopfsalat", "rucola", "lauch", "sellerie", "kürbis", "aubergine",
               "erbsen", "bohnen", "mais", "pilze", "champignon", "kartoffel",
               "süßkartoffel", "fenchel", "kohlrabi", "rote bete", "radieschen"],
    "Milchprodukte": ["milch", "butter", "joghurt", "käse", "quark", "sahne",
                      "schmand", "frischkäse", "mozzarella", "parmesan", "gouda",
                      "emmentaler", "brie", "feta", "ricotta", "crème fraîche",
                      "buttermilch", "kefir", "skyr"],
    "Fleisch & Fisch": ["hähnchen", "hühnchen", "rind", "schwein", "lamm", "kalb",
                         "hackfleisch", "steak", "schnitzel", "wurst", "speck",
                         "schinken", "lachs", "thunfisch", "garnelen", "shrimps",
                         "fisch", "forelle", "zander", "kabeljau", "dorade"],
    "Brot & Backwaren": ["brot", "brötchen", "toast", "baguette", "croissant",
                          "laugenbrezel", "knäckebrot", "mehl", "hefe", "backpulver"],
    "Getränke": ["wasser", "saft", "cola", "bier", "wein", "kaffee", "tee",
                 "limo", "limonade", "mineralwasser", "orangensaft", "apfelsaft",
                 "smoothie", "milchmix"],
    "Gewürze & Öle": ["salz", "pfeffer", "öl", "olivenöl", "essig", "senf",
                       "ketchup", "mayo", "mayonnaise", "sojasoße", "zucker",
                       "honig", "zimt", "paprikapulver", "curry", "chili",
                       "thymian", "rosmarin", "basilikum", "oregano", "petersilie",
                       "schnittlauch", "lorbeer", "muskat", "vanille"],
    "Tiefkühl": ["tiefkühl", "tk-", "gefroren", "frozen", "eis", "eiscreme"],
    "Konserven & Trockenware": ["dose", "konserve", "nudeln", "pasta", "spaghetti",
                                 "penne", "reis", "linsen", "kichererbsen", "bohnen",
                                 "tomatenmark", "passierte tomaten", "kokosmilch",
                                 "müsli", "haferflocken", "cornflakes"],
    "Haushalt": ["küchenrolle", "toilettenpapier", "spülmittel", "waschmittel",
                  "reiniger", "müllbeutel", "folie", "backpapier", "schwamm"],
}
...
class ShoppingService:
    ...
    async def remove_item(self, user_key: str, item_id: int) -> bool:
        ...
        with get_db()() as session:
            item = session.query(ShoppingItem).filter_by(
                id=item_id, user_key=user_key
            ).first()
            if not item:
                return False
            session.delete(item)
    ...
    async def get_items(
        self, user_key: str, include_checked: bool = False
    ) -> list[dict]:
        """
    ...
    async def clear_checked(self, user_key: str) -> int:
        ...
        count = 0
        with get_db()() as session:
            items = session.query(ShoppingItem).filter_by(
                user_key=user_key, checked=True
            ).all()
            count = len(items)
            for item in items:
                session.delete(item)
