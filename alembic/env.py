"""Alembic env.py – liest DB-URL aus config/settings.py."""
...
# Projekt-Root in sys.path, damit config + src importierbar sind
sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

from config.settings import settings  # noqa: E402
from src.services.database import Base  # noqa: E402

config = context.config

# DB-URL dynamisch aus Settings setzen
...
def run_migrations_online() -> None:
    ...
    with connectable.connect() as connection:
        # Bestehende Basis-Tabellen (user_profiles, scanned_documents etc.)
